...
class DireccionesController extends AppController
{
	...
	public function add()
	{
		...
		if ( $this->request->is('post') )
		{
			...
			if(!empty($this->request->data['Sucursal'])){

				$this->Session->write('Flujo.Carro.retiro_tienda', 1);
				$this->Session->write('Flujo.Carro.tienda_retiro', $this->request->data['Sucursal']['sucursal_id']);
		...
		$tipo_header = 'despacho';
		$productos			= $this->Carro->productos('catalogo');
		$carro				= $this->Carro->estado();

		$tipoDirecciones			= $this->Direccion->TipoDireccion->find('list');

		$this->set(compact('regiones', 'direcciones', 'lista', 'tiposDespacho', 'sucursales', 'tipo_header', 'productos', 'carro', 'tipoDirecciones'));
	}
