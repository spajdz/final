...
class UsuariosController extends AppController
{
	public function edit()
	{
		...
		$FotoActual = "";
		// prx($this->request->data);
		if ($this->request->data['Usuario']['imagen']) {
			$FotoActual = $this->request->data['Usuario']['imagen']['mini'];
		}
		BreadcrumbComponent::add('Mis Datos');
		$this->set(compact('tipoUsuarios', 'FotoActual'));
	}
		...
		if ( $this->request->is('post') )
		{
			if ( ! empty($this->request->data['Usuario']['email']) && ( $usuario = $this->Usuario->findByEmail($this->request->data['Usuario']['email']) ) )
			{
				...
				if ( $this->Usuario->saveField('codigo', $hash) )
				{
					...
					$evento			= new CakeEvent('Model.Usuario.afterSave.recuperar', $this, $usuario);
	...
	public function cambiar_clave()
	{
		...
		BreadcrumbComponent::add('Cambio de contraseña');
	}
