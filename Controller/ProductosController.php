...
{
	public $uses = array('Producto', 'Categoria', 'Carga', 'ProductosCarga','Banner', 'ImagenProducto');
	// ZSMOTOR
	...
	public function index($categoria = null){
		...
		if($esMayorista){
			$condiciones['Producto.preciofinal_mayorista >'] = 10;
		}else{
			$condiciones['Producto.preciofinal_publico >'] = 10;
		}
		...
		BreadcrumbComponent::add($bread);

		$this->set(compact('productos', 'categoria', 'categoria_id', 'limite', 'buscador_tipo'));
	}
	...
	public function categoria($slugcat=null, $slugsubcat = null){
		...
		BreadcrumbComponent::add(ucwords($categoria), '/'.$categoria);

		if(!empty($slugcat)){
			BreadcrumbComponent::add(str_replace('-', ' ', ucwords($slugcat)) , '/categoria/'.$slugcat);
			if(!empty($slugsubcat)){
				BreadcrumbComponent::add(str_replace('-', ' ', ucwords($slugsubcat)) , '/categoria/'.$slugcat.'/'.$slugsubcat);
			}
		}
	...
	public function filtros(){
		...
		BreadcrumbComponent::add('Búsqueda');
		$this->set(compact('productos', 'categoria', 'limite', 'texto_busqueda'));
	}
	...
	public function view($categoria_slug = null, $marca_slug=null, $producto_slug = null)
    {
		...
		$imagenes = $this->ImagenProducto->find('all', array('conditions' => array('ImagenProducto.producto_id' => $producto['Producto']['id'])));
		...
		BreadcrumbComponent::add(ucwords($categoria),'/'.$categoria);
		if(!empty($categoria) && $categoria == 'accesorios'){
			BreadcrumbComponent::add($producto['Categoria']['Parent']['nombre'],'/categoria/'.strtolower($producto['Categoria']['Parent']['slug']));
			BreadcrumbComponent::add($producto['Categoria']['nombre'],'/categoria/'.strtolower($producto['Categoria']['Parent']['slug']).'/'.strtolower($producto['Categoria']['slug']));
		}
	...
	public function carro()
	{
		...
		$this->set(compact('productos', 'carro', 'tipo_header'));
	}
	...
	public function admin_edit($id = null)
	{
		...
		if ( $this->request->is('post') || $this->request->is('put') )
		{
			...
			if(empty($producto['Producto']['imagen']['name'])){
				unset($producto['Producto']['imagen']);
			}
			
			foreach ($producto['ImagenProducto'] as $index => $imagen) {
				if ($imagen['imagen']['error'] > 0) {
					unset($producto['ImagenProducto'][$index]);
				}
			}

			$ImagenesEliminadas = explode(',',$producto['imageneseliminadas']);
			if(!empty($ImagenesEliminadas)){
				unset($producto['imageneseliminadas']);
			}

			// prx($ImagenesEliminadas);
			
			if($this->Producto->saveAll($producto)){
				if ($ImagenesEliminadas != "") {
					$this->ImagenProducto->deleteAll(array('ImagenProducto.id' => $ImagenesEliminadas), false);

					// $dbo = $this->ImagenProducto->getDatasource();
  			// 		$logs = $dbo->getLog();
  			...
  			// 		prx($logs);
					...
					for ($i = 0; $i < count($ArrayEliminadas); $i++) {
						$DirEliminada = new Folder(WWW_ROOT . "img/ImagenProducto/" .$ArrayEliminadas[$i]. "/");
						$DirEliminada->delete();
					}
			...
			$this->request->data	= $this->Producto->find('first', array(
				...
				'contain'	=> array('Marca', 'Categoria')
			));
		}

		$sku = $this->request->data['Producto']['sku'];
		$modified = $this->request->data['Producto']['modified'];
		

		
		// $FotoActual = "";
		// prx($this->request->data);
		// if ($this->request->data['Producto']['imagen']) {
		// 	$FotoActual = $this->request->data['Producto']['imagen']['mini'];
		// }

		$ImagenesAsociadas = $this->ImagenProducto->find('all', array(
			'conditions' => array(
				'producto_id' => $id
			),
		));
		$this->set(compact( 'FotoActual','sku', 'modified','ImagenesAsociadas'));
	}
	...
	public function index_hookipa()
	{
		 ...
		 */
		$limite				= ($this->Session->check('Catalogo.Preferencias.limite') ? $this->Session->read('Catalogo.Preferencias.limite') : 10);
		if ( isset($this->params->params['named']['limite']) && in_array($this->params->params['named']['limite'], array(10, 20,30, 40)) )
		{
			$limite				= $this->params->params['named']['limite'];
	...
	public function ajax_eliminar_hookipa()
	{
		$this->layout	= 'ajax';
		$data			= array('success' => false);
	...
	public function admin_edit_hookipa($id = null)
	{
		if ( ! $this->Producto->exists($id) )
		{
			$this->Session->setFlash('Registro inválido.', null, array(), 'danger');
